...
def process_csv(input_file, output_file):
    ...
    # Add SpeedupSym columns for both OMP and MPI DT
    data['SpeedupSym'] = None
    for index, row in data.iterrows():
        if row['Version'] in ['OMP', 'MPI DT']:
    ...
    # Add SpeedupMatTrans columns for both OMP and MPI DT
    ...
    # Add EfficiencySym column for both OMP and MPI DT
    data['EfficiencySym'] = data.apply(
        lambda row: (float(row['SpeedupSym']) / row['Threads/Proc']) * 100
        if row['SpeedupSym'] is not None and row['Version'] in ['OMP', 'MPI DT'] else None,
        axis=1
    )

    # Add EfficiencyMatTrans column for both OMP and MPI DT
    data['EfficiencyMatTrans'] = data.apply(
        lambda row: (float(row['SpeedupMatTrans']) / row['Threads/Proc']) * 100
        if row['SpeedupMatTrans'] is not None and row['Version'] in ['OMP', 'MPI DT'] else None,
        axis=1
    )
