...
def plot_and_save_scaling(input_file, output_dir):
    ...
    required_columns = ['Processes', 'scaling_RollingMean_MatTranspose', 'scaling_RollingMean_CheckSym']
    if not all(col in df.columns for col in required_columns):
        raise ValueError("Il file deve contenere le colonne 'Processes', 'scaling_RollingMean_MatTranspose' e 'scaling_RollingMean_CheckSym'.")
    ...
    for i in range(len(start_indices) - 1):
        ...
        # Grafico con entrambe le colonne e linea di sfondo
        plt.figure(figsize=(12, 7))
        plt.plot(segment['Processes'], segment['scaling_RollingMean_MatTranspose'], marker='o', label='scaling_RollingMean_MatTranspose')
        plt.plot(segment['Processes'], segment['scaling_RollingMean_CheckSym'], marker='s', label='scaling_RollingMean_CheckSym')
        plt.plot(segment['Processes'], inverse_bg, linestyle='--', color='gray', label='Decremento Inverso (1/x)')

        plt.title(f'Scaling Comparison - Potenza di 2: {current_power}')
        ...
        # Salvare il grafico
        output_path = os.path.join(output_dir, f'scaling_comparison_potenza_{current_power}.png')
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Genera e salva grafici comparativi per "scaling_RollingMean_MatTranspose" e "scaling_RollingMean_CheckSym" con decremento inverso (1/x).')
