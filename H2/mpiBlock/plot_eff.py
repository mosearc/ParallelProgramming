    ...
    for dim in data['Dim'].unique():
        ...
        for column in columns_to_plot:
            if not omp_data.empty:
                plt.plot(omp_data['Processes'], omp_data[column], label=f'MPI BDV - {column}', marker='o', linestyle='-', linewidth=2)
            if not mpi_data.empty:
                plt.plot(mpi_data['Processes'], mpi_data[column], label=f'MPI DT - {column}', marker='s', linestyle='--', linewidth=2)
